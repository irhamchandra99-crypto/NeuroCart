...
import Opening from "@/components/Opening"; // ← TAMBAHAN
...
export default function HomePage() {
  ...
  const [ready, setReady]     = useState(false); // ← TAMBAHAN
  ...
  return (
    <>
      {/* ── OPENING — render di atas segalanya ── */}
      <Opening onComplete={() => setReady(true)} /> {/* ← TAMBAHAN */}

      {/* ── KONTEN UTAMA — fade in setelah opening selesai ── */}
      <div style={{
        opacity:    ready ? 1 : 0,          // ← TAMBAHAN
        transition: "opacity 0.6s ease",    // ← TAMBAHAN
        minHeight: "100vh",
          ...
          <div style={{ maxWidth: "1200px", margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px", padding: "32px 0" }}>
            {STATS.map((s, i) => (
              <motion.div key={s.label}
                ...
                style={{ padding: "36px 40px", background: "rgba(255,255,255,0.03)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.06)", position: "relative", borderRadius: "12px" }}
              >
        ...
        <section style={{ padding: "96px 24px", borderTop: "1px solid #0f0f0f", background: "rgba(5,5,5,0.7)", backdropFilter: "blur(12px)", textAlign: "center" }}>
          <div style={{ maxWidth: "1000px", margin: "0 auto", width: "100%" }}>
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              ...
              <h2 style={{ fontSize: "clamp(36px, 8vw, 80px)", fontWeight: 900, letterSpacing: "-0.02em", fontFamily: "var(--font-syne), sans-serif", lineHeight: 0.9, margin: "0 0 28px", textAlign: "center", display: "block", width: "100%" }}>
        ...
        <footer style={{ borderTop: "1px solid #0f0f0f", padding: "20px 48px", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(5,5,5,0.9)",  }}>
          <span style={{ fontSize: "10px", color: "#8f8b8bff", fontFamily: "monospace", letterSpacing: "0.15em" }}>NEUROCART v1.0 · BASE SEPOLIA · CHAINLINK</span>
