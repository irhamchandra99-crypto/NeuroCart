      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          {agent.activeJobs > 0 && (
            ...
            >{agent.activeJobs} RUNNING</motion.span>
          )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "14px", borderTop: `1px solid ${T.border.subtle}` }}>
        <div style={{ display: "flex", gap: "20px" }}>
          <div>
            ...
            <div style={{ fontSize: "14px", fontWeight: 700, fontFamily: "monospace", color: T.text.accent }}>{agent.priceDisplay}</div>
          </div>
          <div>
...
function FilterBar({ filters, setFilters, allSkills, counts }: {
  ...
  allSkills: string[]; counts: { all: number; active: number; offline: number };
}) {
  return (
    <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", alignItems: "center", padding: "16px 20px", background: T.bg.card, borderRadius: `${T.radius.card} ${T.radius.card} 0 0`, borderBottom: `1px solid ${T.border.default}` }}>
      {([["all","ALL",counts.all],["active","ACTIVE",counts.active],["offline","OFFLINE",counts.offline]] as const).map(([val, label, count]) => (
      ...
      {([["rep","REPUTATION"],["jobs","JOBS"],["price","PRICE"]] as const).map(([val, label]) => (
  ...
  return (
    ...
    >
      <motion.div
        ...
        exit={{ y: 20, opacity: 0 }} transition={{ duration: 0.28, ease: [0.16, 1, 0.3, 1] }}
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "24px" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
              ...
              />
              <span style={{ fontSize: "10px", color: agent.isActive ? T.text.muted : T.text.disabled, fontFamily: "monospace", letterSpacing: "0.2em" }}>{agent.isActive ? "ACTIVE" : "OFFLINE"}</span>
              <span style={{ fontSize: "9px", padding: "2px 8px", background: `${tier.color}15`, border: `1px solid ${tier.color}30`, color: tier.color, fontFamily: "monospace", borderRadius: T.radius.badge }}>{tier.label}</span>
              <span style={{ fontSize: "10px", color: T.text.disabled, fontFamily: "monospace" }}>· ERC-8004</span>
            </div>
            <p style={{ fontSize: "10px", color: tier.color, fontFamily: "monospace", letterSpacing: "0.2em", marginBottom: "8px", opacity: 0.7 }}>AGENT #{String(agent.id).padStart(3, "0")}</p>
            <h2 style={{ fontSize: "28px", fontWeight: 900, letterSpacing: "-0.02em", margin: 0, fontFamily: "var(--font-syne), sans-serif" }}>{agent.name}</h2>
          </div>
          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} onClick={onClose}
            style={{ background: T.bg.elevated, border: `1px solid ${T.border.default}`, color: T.text.secondary, cursor: "pointer", fontSize: "16px", width: "36px", height: "36px", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, borderRadius: "8px" }}
          >×</motion.button>
        </div>
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
            {agent.skills.map((skill) => (
              <span key={skill} style={{ fontSize: "11px", padding: "5px 12px", background: "rgba(74,222,128,0.05)", border: "1px solid rgba(74,222,128,0.15)", color: T.text.muted, fontFamily: "monospace", borderRadius: T.radius.badge }}>{skill}</span>
            ))}
          </div>
        </div>
...
export default function AgentsPage() {
  ...
  // ✅ FIX: tidak tergantung HAS_CONTRACTS
  const useRealData = !!REGISTRY_ADDRESS && isConnected;
...
function AgentsContent({ useRealData, selectedAgent, setSelectedAgent, filters, setFilters }: {
  useRealData: boolean; selectedAgent: AgentUI | null;
  ...
  const agents: AgentUI[] = useMemo(() => {
    // ✅ FIX: kalau useRealData tapi data kosong, return [] bukan MOCK
    if (!useRealData || !agentBatchData || agentBatchData.length === 0) {
      return useRealData ? [] : MOCK_AGENTS;
    }
  ...
  const counts = { all: agents.length, active: agents.filter((a) => a.isActive).length, offline: agents.filter((a) => !a.isActive).length };
    ...
    <div style={{ minHeight: "100vh", background: "transparent", color: "white", fontFamily: "var(--font-space), sans-serif" }}>
      ...
      <section style={{ maxWidth: "1200px", margin: "0 auto", padding: "40px 48px" }}>
        ...
        <div style={{ display: "flex", gap: "20px", marginTop: "16px", flexWrap: "wrap" }}>
          {[{ label: "ELITE ≥ 90", color: "#4ade80" }, { label: "PRO ≥ 80", color: "#4ade80" }, { label: "FAIR ≥ 60", color: "#fbbf24" }, { label: "NEW < 60", color: "#f87171" }].map((t) => (
